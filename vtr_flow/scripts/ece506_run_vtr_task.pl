...
sub run_single_task {
	...
	my @archs;
	my $cmos_tech_path = "";
	#arpit channel_width_list_add array
	my @cw_list_add;	
	my $task     = shift(@_);
	...
	while (<CONFIG_FH>) {
		...
		#arpit parse individual channel width  
		elsif ( $key eq "channel_width_list_add" ) {
			push(@cw_list_add, $value);
		}
		...
		if ( $processors == 1 ) {
			foreach my $circuit (@circuits) {
				foreach my $arch (@archs) {
					...
					system(
						#arpit
						"$script_path $circuits_dir/$circuit $archs_dir/$arch -sdc_file $sdc_dir/$sdc $script_params -vpr_route_chan_width @cw_list_add[$cw_counter]\n"
					);
				}
			$cw_counter++;
			}
